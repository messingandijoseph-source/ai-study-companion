...
// Added WidgetsBindingObserver to detect app open/close
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _focusService.startShield(); // Auto-activate shield on app entry
  }

  @override
  void dispose() {
    WidgetsBinding.instance.removeObserver(this); // Clean up observer
    _focusService.stopShield(); // Ensure shield is off when app is destroyed
    super.dispose();
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      _focusService.startShield(); // App back in foreground -> Block
    } else {
      _focusService.stopShield(); // App minimized -> Allow notifications
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        ).animate(key: ValueKey(index)).fadeIn().slideX(begin: -0.1),
      ),
      body: AnimatedSwitcher(
